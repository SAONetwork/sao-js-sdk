/* eslint-disable */
import _m0 from "protobufjs/minimal";
import { Coin } from "../../cosmos/base/v1beta1/coin";

export const protobufPackage = "saonetwork.sao.node";

export interface PledgeDebt {
  sp: string;
  debt: Coin | undefined;
}

function createBasePledgeDebt(): PledgeDebt {
  return { sp: "", debt: undefined };
}

export const PledgeDebt = {
  encode(message: PledgeDebt, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.sp !== "") {
      writer.uint32(10).string(message.sp);
    }
    if (message.debt !== undefined) {
      Coin.encode(message.debt, writer.uint32(18).fork()).ldelim();
    }
    return writer;
  },

  decode(input: _m0.Reader | Uint8Array, length?: number): PledgeDebt {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBasePledgeDebt();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
          message.sp = reader.string();
          break;
        case 2:
          message.debt = Coin.decode(reader, reader.uint32());
          break;
        default:
          reader.skipType(tag & 7);
          break;
      }
    }
    return message;
  },

  fromJSON(object: any): PledgeDebt {
    return {
      sp: isSet(object.sp) ? String(object.sp) : "",
      debt: isSet(object.debt) ? Coin.fromJSON(object.debt) : undefined,
    };
  },

  toJSON(message: PledgeDebt): unknown {
    const obj: any = {};
    message.sp !== undefined && (obj.sp = message.sp);
    message.debt !== undefined && (obj.debt = message.debt ? Coin.toJSON(message.debt) : undefined);
    return obj;
  },

  fromPartial<I extends Exact<DeepPartial<PledgeDebt>, I>>(object: I): PledgeDebt {
    const message = createBasePledgeDebt();
    message.sp = object.sp ?? "";
    message.debt = (object.debt !== undefined && object.debt !== null) ? Coin.fromPartial(object.debt) : undefined;
    return message;
  },
};

type Builtin = Date | Function | Uint8Array | string | number | boolean | undefined;

export type DeepPartial<T> = T extends Builtin ? T
  : T extends Array<infer U> ? Array<DeepPartial<U>> : T extends ReadonlyArray<infer U> ? ReadonlyArray<DeepPartial<U>>
  : T extends {} ? { [K in keyof T]?: DeepPartial<T[K]> }
  : Partial<T>;

type KeysOfUnion<T> = T extends T ? keyof T : never;
export type Exact<P, I extends P> = P extends Builtin ? P
  : P & { [K in keyof P]: Exact<P[K], I[K]> } & { [K in Exclude<keyof I, KeysOfUnion<P>>]: never };

function isSet(value: any): boolean {
  return value !== null && value !== undefined;
}
