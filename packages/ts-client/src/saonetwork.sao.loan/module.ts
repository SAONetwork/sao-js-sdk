...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgWithdraw({ value }: msgWithdrawParams): EncodeObject {
			try {
				return { typeUrl: "/saonetwork.sao.loan.MsgWithdraw", value: MsgWithdraw.fromPartial( value ) }  
			} catch (e: any) {
		...
		msgDeposit({ value }: msgDepositParams): EncodeObject {
			try {
				return { typeUrl: "/saonetwork.sao.loan.MsgDeposit", value: MsgDeposit.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			SaonetworkSaoLoan: new SDKModule(test)
		},
		registry: msgTypes
  }
}
