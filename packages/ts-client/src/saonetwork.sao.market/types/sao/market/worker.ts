...
function createBaseWorker(): Worker {
  return { workername: "", storage: 0, reward: undefined, incomePerSecond: undefined, lastRewardAt: 0 };
}
...
export const Worker = {
  encode(message: Worker, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.incomePerSecond !== undefined) {
      DecCoin.encode(message.incomePerSecond, writer.uint32(34).fork()).ldelim();
    }
    if (message.lastRewardAt !== 0) {
      writer.uint32(40).int64(message.lastRewardAt);
    }
    return writer;
  },
